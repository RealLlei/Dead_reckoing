 ...
 */
class alignas(CACHELINE_SIZE) LaneChangePrefinishEvaluator
    : public SpeedEvaluator {
 public:
  explicit LaneChangePrefinishEvaluator(const SpeedEvaluatorConfig& config);
 ...
 private:
   ...
   */
  void ProcessCommonScenario(const Frame& frame,
   ...
   * @return const std::vector<std::shared_ptr<SpeedCost>>& 
   */
  void ProcessMergeScenario(const Frame& frame,
                            ReferenceLineInfo* reference_line_info);
  ...
  LaneChangePrefinishEvaluatorConfig config_;
};
