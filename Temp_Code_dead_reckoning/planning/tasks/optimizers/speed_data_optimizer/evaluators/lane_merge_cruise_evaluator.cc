...
LaneMergeCruiseEvaluator::LaneMergeCruiseEvaluator(
    const SpeedEvaluatorConfig& config)
    : SpeedEvaluator(config),
      config_(config.lane_merge_cruise_evaluator_config()) {
...
void LaneMergeCruiseEvaluator::CalculateCost(
...
bool LaneMergeCruiseEvaluator::CheckFallbackSpeedData(
    const SpeedCache& cache, const ReferenceLineInfo& reference_line_info,
...
void LaneMergeCruiseEvaluator::CalculateModeCost(
...
void LaneMergeCruiseEvaluator::CalculateTotalCost(
    const SpeedCurveCostResult* cost_result1,
...
bool LaneMergeCruiseEvaluator::CompareCostResult(
    const SpeedCurveCostResult* cost_result1,
...
bool LaneMergeCruiseEvaluator::CompareCurrentCostResult(
    const SpeedCurveCostResult* cost_result1,
    const SpeedCurveCostResult* cost_result2) const {
  if (cost_result1 == nullptr || cost_result1->curve == nullptr) {
    return false;
...
bool LaneMergeCruiseEvaluator::CompareCurrentAndLastCostResult(
