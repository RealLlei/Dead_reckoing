...
LaneKeepEvaluator::LaneKeepEvaluator(const SpeedEvaluatorConfig& config)
    : SpeedEvaluator(config), config_(config.lane_keep_evaluator_config()) {
...
void LaneKeepEvaluator::CalculateCost(
...
bool LaneKeepEvaluator::CheckFallbackSpeedData(
    const SpeedCache& cache, const ReferenceLineInfo& reference_line_info,
...
void LaneKeepEvaluator::CalculateModeCost(
...
void LaneKeepEvaluator::CalculateTotalCost(
    const SpeedCurveCostResult* cost_result1,
...
bool LaneKeepEvaluator::CompareCostResult(
    const SpeedCurveCostResult* cost_result1,
...
bool LaneKeepEvaluator::CompareCurrentCostResult(
    const SpeedCurveCostResult* cost_result1,
    const SpeedCurveCostResult* cost_result2) const {
  if (cost_result1 == nullptr || cost_result1->curve == nullptr) {
    return false;
...
bool LaneKeepEvaluator::CompareCurrentAndLastCostResult(
