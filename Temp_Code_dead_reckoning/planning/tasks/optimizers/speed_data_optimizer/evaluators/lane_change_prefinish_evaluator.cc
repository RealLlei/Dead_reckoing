...
LaneChangePrefinishEvaluator::LaneChangePrefinishEvaluator(
    const SpeedEvaluatorConfig& config)
    : SpeedEvaluator(config),
      config_(config.lane_change_prefinish_evaluator_config()) {
...
void LaneChangePrefinishEvaluator::CalculateCost(
...
bool LaneChangePrefinishEvaluator::CheckFallbackSpeedData(
    const SpeedCache& cache, const ReferenceLineInfo& reference_line_info,
...
void LaneChangePrefinishEvaluator::CalculateModeCost(
...
void LaneChangePrefinishEvaluator::CalculateTotalCost(
    const SpeedCurveCostResult* cost_result1,
...
bool LaneChangePrefinishEvaluator::CompareCostResult(
    const SpeedCurveCostResult* cost_result1,
...
bool LaneChangePrefinishEvaluator::CompareCurrentCostResult(
    const SpeedCurveCostResult* cost_result1,
    const SpeedCurveCostResult* cost_result2) const {
  if (cost_result1 == nullptr || cost_result1->curve == nullptr) {
    return false;
...
bool LaneChangePrefinishEvaluator::CompareCurrentAndLastCostResult(
