...
LaneContinueEvaluator::LaneContinueEvaluator(const SpeedEvaluatorConfig& config)
    : SpeedEvaluator(config), config_(config.lane_continue_evaluator_config()) {
...
void LaneContinueEvaluator::CalculateCost(
...
bool LaneContinueEvaluator::CheckFallbackSpeedData(
    const SpeedCache& cache, const ReferenceLineInfo& reference_line_info,
...
void LaneContinueEvaluator::CalculateModeCost(
...
void LaneContinueEvaluator::CalculateTotalCost(
    const SpeedCurveCostResult* cost_result1,
...
bool LaneContinueEvaluator::CompareCostResult(
    const SpeedCurveCostResult* cost_result1,
...
bool LaneContinueEvaluator::CompareCurrentCostResult(
    const SpeedCurveCostResult* cost_result1,
    const SpeedCurveCostResult* cost_result2) const {
  if (cost_result1 == nullptr || cost_result1->curve == nullptr) {
    return false;
...
bool LaneContinueEvaluator::CompareCurrentAndLastCostResult(
