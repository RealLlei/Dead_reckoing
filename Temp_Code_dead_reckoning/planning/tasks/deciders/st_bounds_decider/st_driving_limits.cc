...
std::pair<double, double> STDrivingLimits::GetVehicleDynamicsLimits(
  ...
  if (dynamic_limits.first < 1e-3) {
    dynamic_limits.first = 0.0;
  }
...
void STDrivingLimits::ChangeMaxDece(double acce) {
  if (acce > 0) {
    max_dec_ += acce;  // max_dece_ is a positive value.
  } else {
    AERROR << "can't add a negative value on max_dec_ because the max_dec_ is "
              "positive value.";
  }
}
