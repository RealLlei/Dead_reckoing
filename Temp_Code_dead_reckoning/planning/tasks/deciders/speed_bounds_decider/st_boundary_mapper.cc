...
Status STBoundaryMapper::ComputeSTBoundary(PathDecision* path_decision) const {
  ...
  for (const auto* ptr_obstacle_item : path_decision->obstacles().Items()) {
    ...
    if (decision.has_stop()) {
      ...
      const double stop_s = path_data_.frenet_frame_path().is_forward_path()
                                ? stop_sl_point.s()
                                : -stop_sl_point.s();
...
bool STBoundaryMapper::MapStopDecision(
  ...
  const double stop_ref_s =
      path_data_.frenet_frame_path().is_forward_path()
          ? stop_sl_point.s() - vehicle_param_.front_edge_to_center()
          : stop_sl_point.s() + vehicle_param_.back_edge_to_center();

  if (path_data_.frenet_frame_path().is_forward_path()) {
    ...
    } else if (stop_ref_s < path_data_.frenet_frame_path().front().s()) {
      st_stop_s = path_data_.discretized_path().front().s() +
                  (stop_ref_s - path_data_.frenet_frame_path().front().s());
    } else {
    ...
    if (stop_ref_s < path_data_.frenet_frame_path().back().s()) {
      st_stop_s = path_data_.discretized_path().back().s() +
                  (path_data_.frenet_frame_path().back().s() - stop_ref_s);
    } else if (stop_ref_s > path_data_.frenet_frame_path().front().s()) {
      st_stop_s = path_data_.discretized_path().front().s() +
                  (path_data_.frenet_frame_path().front().s() - stop_ref_s);
    } else {
      PathPoint stop_point;
      if (!path_data_.GetPathPointWithRefS(stop_ref_s, &stop_point)) {
        return false;
  ...
  stop_obstacle->set_path_st_boundary(std::move(boundary));
  return true;
}
...
void STBoundaryMapper::ComputeSTBoundary(Obstacle* obstacle) const {
  ...
  obstacle->set_path_st_boundary(std::move(boundary));
}
...
bool STBoundaryMapper::GetOverlapBoundaryPoints(
  ...
  if (trajectory.trajectory_point().empty()) {
    ...
    const std::size_t default_num_point = 50;
    ...
    for (int i = 0; i < trajectory.trajectory_point_size(); ++i) {
      ...
      const auto s_count = static_cast<int>(std::round(path_len / step_length));
      for (int i = 0; i <= s_count; ++i) {
        const auto path_s = i * step_length;
