...
Status PathReuseDecider::Process(Frame* const frame,
  ...
  const auto scenario_type = injector_->planning_context()
                                 ...
                                 .scenario()
                                 .scenario_type();
  if (scenario_type != ScenarioStatus::NNP_LANE_FOLLOW &&
      scenario_type != ScenarioStatus::NCP_LANE_FOLLOW) {
    ADEBUG << "skipping reusing path: not in NNP_LANE_FOLLOW scenario";
    reference_line_info->set_path_reusable(false);
    return Status::OK();
  }
...
bool PathReuseDecider::IsIgnoredBlockingObstacle(
  ...
  double blocking_obstacle_start_s = 0.0;
...
bool PathReuseDecider::IsCollisionFree(
  ...
  for (size_t i = 0; i < history_path->size(); ++i) {
    ...
    common::math::Vec2d path_position = {history_path->at(i).x(),
                                         history_path->at(i).y()};
    ...
    const auto& vehicle_box =
        common::VehicleConfigHelper::GetBoundingBox(history_path->at(i));
    std::vector<Vec2d> ABCDpoints = vehicle_box.GetAllCorners();
    std::vector<Vec2d> ABCDpoints_sl;
    for (const auto& corner_point : ABCDpoints) {
      ...
      ABCDpoints_sl.emplace_back(curr_point_sl.s(), curr_point_sl.l());
    }
    Polygon2d ADC_vehicle_polygon = Polygon2d(ABCDpoints_sl);
    // Check if it's in any polygon of other static obstacles.
    for (const auto& obstacle_polygon : obstacle_polygons) {
      if (obstacle_polygon.HasOverlap(ADC_vehicle_polygon)) {
        // for debug
        ADEBUG << "s distance to end point:" << path_end_position_sl.s();
        ADEBUG << "s distance to end point:" << path_position_sl.s();
        ADEBUG << "[" << i << "]"
               << ", history_path[i].x(): " << history_path->at(i).x()
               << ", history_path[i].y()" << history_path->at(i).y();
        ADEBUG << "collision:" << path_position.x() << ", "
               << path_position.y();
...
bool PathReuseDecider::NotShortPath(const DiscretizedPath& current_path) {
  // TODO(shu): use gflag
  static constexpr size_t kShortPathThreshold = 350;
...
bool PathReuseDecider::TrimHistoryPath(
  ...
  // ADEBUG << "init_path_point x:[" << FIXED << SETPRECISION(3) <<
  // init_path_point.x()
  //        << "], y[" << init_path_point.y() << "], s: [" <<
  //        init_path_point.s()
  //        << "]";
  ...
  for (size_t i = path_start_index; i < history_path->size(); ++i) {
    ...
    trimmed_path.emplace_back(history_path->at(i));

    // if (i < 50) {
    //   ADEBUG << "path_point:[" << i << "]" << updated_s;
    //   path_position_sl.s();
    //   ADEBUG << FIXED << SETPRECISION(3)<< "path_point:[" << i << "]"
  ...
  // set path
  auto* const path_data = reference_line_info->mutable_path_data();
