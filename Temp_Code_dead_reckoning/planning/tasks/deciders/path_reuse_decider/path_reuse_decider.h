...
class PathReuseDecider : public Decider {
 ...
 private:
  ...
  bool GetBlockingObstacleS(ReferenceLineInfo* reference_line_info,
                            double* blocking_obstacle_s);
  // ignore blocking obstacle when it is far away
  bool IsIgnoredBlockingObstacle(ReferenceLineInfo* reference_line_info);
  // check if path is collision free
  bool IsCollisionFree(ReferenceLineInfo* reference_line_info);

  // check path length
  static bool NotShortPath(const DiscretizedPath& current_path);

  // trim history path
  bool TrimHistoryPath(Frame* frame, ReferenceLineInfo* reference_line_info);
