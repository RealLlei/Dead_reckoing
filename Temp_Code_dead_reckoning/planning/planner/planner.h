...
class Planner {
 public:
  ...
  virtual TL::common::Status Init(const PlanningConfig& config) = 0;
...
class PlannerWithReferenceLine : public Planner {
 public:
   ...
   * @brief Destructor
   */
  virtual ~PlannerWithReferenceLine() = default;  // NOLINT
   ...
   * @return OK if planning succeeds; error otherwise.
   */
  virtual TL::common::Status PlanOnReferenceLine(
      const common::TrajectoryPoint& planning_init_point, Frame* frame,
