...
class Delay {
 public:
   ...
   */
  T Deal(const T& input, size_t delay_period);
  /**
   ...
   * 返回已经存储的数据的大小
   */
  size_t GetDataSize() { return deque_data_.size(); }
...
template <typename T>
T Delay<T>::Deal(const T& input, size_t delay_period) {
  if (deque_data_.size() < size_) {
    deque_data_.push_front(input);
  } else {
    deque_data_.pop_back();
