...
PublishableTrajectory::PublishableTrajectory(const ADCTrajectory& trajectory_pb)
    : DiscretizedTrajectory(trajectory_pb),
      header_time_(trajectory_pb.header().data_stamp()) {}
...
void PublishableTrajectory::PopulateTrajectoryProtobuf(
  ...
  trajectory_pb->mutable_header()->set_data_stamp(header_time_);
  trajectory_pb->mutable_trajectory_point()->CopyFrom(trajectory_);
  if (!trajectory_.empty()) {
    const auto last_tp = trajectory_.rbegin();
