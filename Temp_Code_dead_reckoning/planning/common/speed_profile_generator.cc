...
SpeedData SpeedProfileGenerator::GenerateFallbackSpeed(
    const EgoInfo* ego_info, double max_dece, const double stop_distance,
    const bool is_forward_plan) {
  AERROR << "Fallback using piecewise jerk speed!";
  const double init_v = is_forward_plan ? ego_info->start_point().v()
                                        : -ego_info->start_point().v();
  const double init_a = is_forward_plan ? ego_info->start_point().a()
                                        : -ego_info->start_point().a();
  ...
  piecewise_jerk_problem.SetXRef(1.0, std::move(end_state_ref));

  piecewise_jerk_problem.SetScaleFactor({1.0, 10.0, 100.0});

  if (TL::common::math::double_type::AlmostEqual(max_dece, 0.0)) {
    max_dece = common::VehicleConfigHelper::GetConfig()
                   .vehicle_param()
                   .max_deceleration();
  }
  double dynamic_stop_distance =
      fmax(init_v * FLAGS_fallback_total_time -
               fabs(max_dece / 2 * FLAGS_fallback_total_time),
           init_v * init_v / (fabs(2 * max_dece)));
  double slack_stop_dis = 3;
  piecewise_jerk_problem.SetXBounds(
      0.0, std::fmax(stop_distance, dynamic_stop_distance + slack_stop_dis));
  piecewise_jerk_problem.SetDxBounds(
      0.0, std::fmax(FLAGS_planning_upper_speed_limit, init_v));
  piecewise_jerk_problem.SetDdxBounds(veh_param.max_deceleration(),
                                      veh_param.max_acceleration());
  piecewise_jerk_problem.SetDddxBounds(FLAGS_longitudinal_jerk_lower_bound,
                                       FLAGS_longitudinal_jerk_upper_bound);

  // Solve the problem
  if (!piecewise_jerk_problem.Optimize("speed_fallback_osqp")) {
    AERROR << "Piecewise jerk fallback speed optimizer failed!";
    return GenerateStopProfile(init_v, init_a, is_forward_plan);
  }
  // Extract output
  const std::vector<double>& s = piecewise_jerk_problem.OptX();
  const std::vector<double>& ds = piecewise_jerk_problem.OptDx();
  const std::vector<double>& dds = piecewise_jerk_problem.OptDdx();
...
void SpeedProfileGenerator::FillEnoughSpeedPoints(SpeedData* const speed_data) {
  const double last_point_s = speed_data->back().s();
  const double last_point_t = speed_data->back().t();
  if (last_point_t >= FLAGS_fallback_total_time) {
    return;
  }
  const auto uint_t = FLAGS_fallback_time_unit;
  const auto step =
      static_cast<int>(std::round(FLAGS_fallback_time_unit / uint_t));
  const auto max_step =
      static_cast<int>(std::round(FLAGS_fallback_total_time / uint_t));
  const auto last_point = static_cast<int>(std::round(last_point_t / uint_t));
  for (int i = last_point + step; i < max_step; i += step) {
    const auto t = i * uint_t;
...
SpeedData SpeedProfileGenerator::GenerateStopProfile(
  ...
  const auto step = FLAGS_fallback_time_unit;
  const auto max_step = static_cast<int>(std::round(max_t / step));
  const auto unit_step = static_cast<int>(std::round(unit_t / step));

  double pre_s = 0.0;
  double pre_v = init_speed;
  double acc = common::VehicleConfigHelper::GetConfig()
                   .vehicle_param()
                   .max_deceleration();
  if (is_forward_plan) {
  ...
  for (int i = unit_step; i < max_step; i += unit_step) {
    const auto t = i * unit_step;
    double s = 0.0;
    double v = 0.0;
    s = std::fmax(pre_s,
