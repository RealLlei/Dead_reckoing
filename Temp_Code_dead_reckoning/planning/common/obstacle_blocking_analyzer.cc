...
double GetDistanceBetweenADCAndObstacle(const Frame& frame,
  ...
  return distance_between_adc_and_obstacle > 0.0
...
double GetDistanceBetweenADCAndObstacle(
  ...
  if ((reference_line_info == nullptr) || (obstacle == nullptr)) {
    AERROR
        << "eithor reference line info or obstalcle pointer is null pointer.";
...
bool IsParkedVehicle(const ReferenceLine& reference_line,
  ...
  if (lanes.size() == 1 &&
      lanes.front()->lane().type() == TL::hdmap::Lane::PARKING) {
