...
using TL::common::math::LineSegment2d;
...
std::string STBoundary::TypeName(BoundaryType type) {
  ...
  if (type == BoundaryType::FOLLOW) {
    ...
    name = "YIELD";
  } else if (type == BoundaryType::NUDGE) {
    name = "NUDGE";
...
bool STBoundary::GetBoundarySRange(const double curr_time, double* s_upper,
  ...
  CHECK_NOTNULL(s_upper);
  CHECK_NOTNULL(s_lower);
  if (TL::common::math::double_type::DefinitelyLess(curr_time, min_t_) ||
      TL::common::math::double_type::DefinitelyGreater(curr_time, max_t_)) {
    return false;
  }
...
void STBoundary::RemoveRedundantPoints(
    std::vector<std::pair<STPoint, STPoint>>* point_pairs) {
  if (point_pairs == nullptr || point_pairs->size() <= 2) {
    return;
  }
...
bool STBoundary::GetIndexRange(const std::vector<STPoint>& points,
                               const double t, size_t* left, size_t* right) {
  CHECK_NOTNULL(left);
  CHECK_NOTNULL(right);
  if (TL::common::math::double_type::DefinitelyLess(t, points.front().t()) ||
      TL::common::math::double_type::DefinitelyGreater(t,
                                                          points.back().t())) {
    AERROR << "t is out of range. t = " << t;
    return false;
  }
  auto comp = [](const STPoint& p, const double t) {
    return TL::common::math::double_type::DefinitelyLess(p.t(), t);
  };
