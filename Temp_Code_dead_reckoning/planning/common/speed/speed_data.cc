...
void SpeedData::AppendSpeedPoint(const double s, const double time,
  ...
  if (!empty() && back().t() < time) {
    ADEBUG << "back_time:" << back().t() << "  time:" << time;
  }
...
std::string SpeedData::DebugString() const {
  const auto limit = static_cast<uint>(std::min(
...
double SpeedData::GetMaxAccel() const {
  const auto iter = std::max_element(
      begin(), end(), [](const auto& speed_point1, const auto& speed_point2) {
        return speed_point1.a() < speed_point2.a();
      });
