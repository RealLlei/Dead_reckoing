...
PathPoint DiscretizedPath::EvaluateForGreaterThanMaxS(
  ...
  const auto x = it_lower->x() + std::cos(it_lower->theta()) * extend_s;
  const auto y = it_lower->y() + std::sin(it_lower->theta()) * extend_s;
  PathPoint path_point;
  path_point.set_x(x);
  path_point.set_y(y);
  path_point.set_z(0.0);
  path_point.set_theta(it_lower->theta());
  path_point.set_kappa(it_lower->kappa());
  path_point.set_dkappa(it_lower->dkappa());
  path_point.set_ddkappa(it_lower->ddkappa());
  path_point.set_s(path_s);
  return path_point;
}
