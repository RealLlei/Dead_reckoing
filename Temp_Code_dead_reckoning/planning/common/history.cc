...
HistoryObjectDecision::GetObjectDecision() const {
  std::vector<const ObjectDecisionType*> result(object_decision_.size());
  for (size_t i = 0; i < object_decision_.size(); i++) {
    result.at(i) = &(object_decision_.at(i));
  }
...
void HistoryFrame::Init(
    const std::shared_ptr<const ADCTrajectory>& adc_trajactory) {
  // adc_trajactory_.CopyFrom(adc_trajactory);

  seq_num_ = static_cast<int>(adc_trajactory->header().seq());
...
std::vector<const HistoryObjectDecision*> HistoryFrame::GetObjectDecisions()
    const {
  std::vector<const HistoryObjectDecision*> result(object_decisions_.size());
  for (size_t i = 0; i < object_decisions_.size(); i++) {
    result.at(i) = &(object_decisions_.at(i));
  }
...
std::vector<const HistoryObjectDecision*> HistoryFrame::GetStopObjectDecisions()
  ...
  for (const auto& orin_decision : object_decisions_) {
    auto obj_decision = orin_decision.GetObjectDecision();
    for (const ObjectDecisionType* decision_type : obj_decision) {
      if (decision_type->has_stop()) {
        ...
        // NOLINTEND
        decision->Init(orin_decision.id(), object_decision);
...
int History::Add(
    const std::shared_ptr<const ADCTrajectory>& adc_trajectory_pb) {
