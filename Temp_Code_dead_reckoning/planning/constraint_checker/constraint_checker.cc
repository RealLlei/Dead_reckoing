...
#include "planning/constraint_checker/constraint_checker.h"
...
ConstraintChecker::Result ConstraintChecker::ValidTrajectory(
    const DiscretizedTrajectory& trajectory, const bool is_forward_trajectory) {
  ...
  for (const auto& p : trajectory) {
    ...
    double lon_v = p.v();
    if (is_forward_trajectory && (!WithinRange(lon_v, FLAGS_speed_lower_bound,
    ...
    } else if (!is_forward_trajectory &&
               (!WithinRange(lon_v, -FLAGS_speed_upper_bound,
                             -FLAGS_speed_lower_bound))) {
      ADEBUG << "Velocity at relative time " << t
             << " exceeds bound, value: " << lon_v << ", bound ["
             << -FLAGS_speed_upper_bound << ", " << -FLAGS_speed_lower_bound
             << "].";
      return Result::LON_VELOCITY_OUT_OF_BOUND;
    }

    double lon_a = p.a();
    if (is_forward_trajectory &&
        (!WithinRange(lon_a, FLAGS_longitudinal_acceleration_lower_bound,
  ...
  for (size_t i = 1; i < trajectory.NumOfPoints(); ++i) {
    ...
    if (is_forward_trajectory &&
        (!WithinRange(lon_jerk, FLAGS_longitudinal_jerk_lower_bound,
      ...
      return Result::LON_JERK_OUT_OF_BOUND;
               ...
               (!WithinRange(lon_jerk, -FLAGS_longitudinal_jerk_upper_bound,
                             -FLAGS_longitudinal_jerk_lower_bound))) {
      ADEBUG << "Longitudinal jerk at relative time " << t
             << " exceeds bound, value: " << lon_jerk << ", bound ["
             << -FLAGS_longitudinal_jerk_upper_bound << ", "
