...
const hdmap::PathOverlap* GetOverlapOnReferenceLine(
 ...
 */
PullOverStatus CheckADCPullOver(const common::VehicleState* vehicle_state,
  ...
  const bool position_check = CheckPullOverPositionBySL(
      ...
      vehicle_state->heading(), target_position, pull_over_status.theta(),
      true);
...
bool CheckPullOverPositionBySL(const ReferenceLineInfo& reference_line_info,
  ...
  double l_diff = 0.0;
  l_diff = std::fabs(target_sl.l() - adc_position_sl.l());
...
bool CheckADCReadyToCruise(const common::VehicleState* vehicle_state,
                           Frame* frame,
                           const ScenarioParkAndGoConfig& scenario_config) {
  const auto* vehicle_status = vehicle_state;
  ...
  return ((vehicle_status->gear() == soc::Chassis::GEAR_DRIVE ||
           std::fabs(vehicle_status->linear_velocity()) < kMinSpeed) &&
