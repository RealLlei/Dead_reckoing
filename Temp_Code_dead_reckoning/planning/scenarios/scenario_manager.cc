...
#include "planning/scenarios/bare_intersection/unprotected/bare_intersection_unprotected_scenario.h"
#include "planning/scenarios/emergency/emergency_pull_over/emergency_pull_over_scenario.h"
#include "planning/scenarios/emergency/emergency_stop/emergency_stop_scenario.h"
#include "planning/scenarios/lane_follow/ncp_lane_follow_scenario.h"
#include "planning/scenarios/lane_follow/nnp_lane_follow_scenario.h"

#include "proto/map/map_lane.pb.h"
// #include "planning/scenarios/learning_model/learning_model_sample_scenario.h"
#include "planning/scenarios/park/pull_over/pull_over_scenario.h"
// #include "planning/scenarios/park_and_go/park_and_go_scenario.h"
// #include "planning/common/planning_gflags.cc"
#include "planning/scenarios/stop_sign/unprotected/stop_sign_unprotected_scenario.h"
#include "planning/scenarios/traffic_light/protected/traffic_light_protected_scenario.h"
#include "planning/scenarios/traffic_light/unprotected_left_turn/traffic_light_unprotected_left_turn_scenario.h"
#include "planning/scenarios/traffic_light/unprotected_right_turn/traffic_light_unprotected_right_turn_scenario.h"
#include "planning/scenarios/util/util.h"
#include "planning/scenarios/valet_parking/valet_parking_scenario.h"
#include "planning/scenarios/yield_sign/yield_sign_scenario.h"
...
ScenarioManager::ScenarioManager(
    ...
    : injector_(injector),
      default_scenario_type_(ScenarioStatus::NNP_LANE_FOLLOW) {}
...
std::unique_ptr<Scenario> ScenarioManager::CreateScenario(
  ...
  switch (scenario_type) {
    case ScenarioStatus::BARE_INTERSECTION_UNPROTECTED:
      ptr = std::make_unique<
          bare_intersection::BareIntersectionUnprotectedScenario>(
          config_map_[scenario_type], &scenario_context_, injector_);
      break;
    case ScenarioStatus::EMERGENCY_PULL_OVER:
      ptr = std::make_unique<emergency_pull_over::EmergencyPullOverScenario>(
          config_map_[scenario_type], &scenario_context_, injector_);
      break;
    case ScenarioStatus::EMERGENCY_STOP:
      ptr = std::make_unique<emergency_stop::EmergencyStopScenario>(
          config_map_[scenario_type], &scenario_context_, injector_);
      break;
    case ScenarioStatus::NNP_LANE_FOLLOW:
      ptr = std::make_unique<lane_follow::NNPLaneFollowScenario>(
          config_map_[scenario_type], &scenario_context_, injector_);
      break;
    case ScenarioStatus::NCP_LANE_FOLLOW:
      ptr = std::make_unique<lane_follow::NCPLaneFollowScenario>(
          config_map_[scenario_type], &scenario_context_, injector_);
      break;
    // case ScenarioStatus::LEARNING_MODEL_SAMPLE:
    //   ptr.reset(new scenario::LearningModelSampleScenario(
    //       config_map_[scenario_type], &scenario_context_, injector_));
    //   break;
    // case ScenarioStatus::PARK_AND_GO:
    //   ptr.reset(new scenario::park_and_go::ParkAndGoScenario(
    //       config_map_[scenario_type], &scenario_context_, injector_));
    //   break;
    case ScenarioStatus::PULL_OVER:
      ptr = std::make_unique<pull_over::PullOverScenario>(
          config_map_[scenario_type], &scenario_context_, injector_);
      break;
    case ScenarioStatus::STOP_SIGN_UNPROTECTED:
      ptr = std::make_unique<stop_sign::StopSignUnprotectedScenario>(
          config_map_[scenario_type], &scenario_context_, injector_);
      break;
    case ScenarioStatus::TRAFFIC_LIGHT_PROTECTED:
      ptr = std::make_unique<traffic_light::TrafficLightProtectedScenario>(
          config_map_[scenario_type], &scenario_context_, injector_);
      break;
    case ScenarioStatus::TRAFFIC_LIGHT_UNPROTECTED_LEFT_TURN:
      ptr = std::make_unique<
          traffic_light::TrafficLightUnprotectedLeftTurnScenario>(
          config_map_[scenario_type], &scenario_context_, injector_);
      break;
    case ScenarioStatus::TRAFFIC_LIGHT_UNPROTECTED_RIGHT_TURN:
      ptr = std::make_unique<
          traffic_light::TrafficLightUnprotectedRightTurnScenario>(
          config_map_[scenario_type], &scenario_context_, injector_);
      break;
    case ScenarioStatus::VALET_PARKING:
      ptr = std::make_unique<valet_parking::ValetParkingScenario>(
          config_map_[scenario_type], &scenario_context_, injector_);
      break;
    case ScenarioStatus::YIELD_SIGN:
      ptr = std::make_unique<yield_sign::YieldSignScenario>(
          config_map_[scenario_type], &scenario_context_, injector_);
      break;
    default:
      return nullptr;
...
void ScenarioManager::RegisterScenarios() {
  // lane_follow
  ACHECK(Scenario::LoadConfig(FLAGS_nnp_scenario_lane_follow_config_file,
                              &config_map_[ScenarioStatus::NNP_LANE_FOLLOW]));
  ACHECK(Scenario::LoadConfig(FLAGS_ncp_scenario_lane_follow_config_file,
                              &config_map_[ScenarioStatus::NCP_LANE_FOLLOW]));

  // bare_intersection
  ACHECK(Scenario::LoadConfig(
      FLAGS_scenario_bare_intersection_unprotected_config_file,
      &config_map_[ScenarioStatus::BARE_INTERSECTION_UNPROTECTED]));

  // emergency_pull_over
  ACHECK(
      Scenario::LoadConfig(FLAGS_scenario_emergency_pull_over_config_file,
                           &config_map_[ScenarioStatus::EMERGENCY_PULL_OVER]));

  // emergency_stop
  ACHECK(Scenario::LoadConfig(FLAGS_scenario_emergency_stop_config_file,
                              &config_map_[ScenarioStatus::EMERGENCY_STOP]));

  // learning model
  // ACHECK(Scenario::LoadConfig(
  //     FLAGS_scenario_learning_model_sample_config_file,
  //     &config_map_[ScenarioStatus::LEARNING_MODEL_SAMPLE]));

  // park_and_go
  // ACHECK(Scenario::LoadConfig(FLAGS_scenario_park_and_go_config_file,
  //                             &config_map_[ScenarioStatus::PARK_AND_GO]));

  // pull_over
  // ACHECK(Scenario::LoadConfig(FLAGS_scenario_pull_over_config_file,
  //                             &config_map_[ScenarioStatus::PULL_OVER]));

  // stop_sign
  ACHECK(Scenario::LoadConfig(
      FLAGS_scenario_stop_sign_unprotected_config_file,
      &config_map_[ScenarioStatus::STOP_SIGN_UNPROTECTED]));

  // traffic_light
  ACHECK(Scenario::LoadConfig(
      FLAGS_scenario_traffic_light_protected_config_file,
      &config_map_[ScenarioStatus::TRAFFIC_LIGHT_PROTECTED]));
  ACHECK(Scenario::LoadConfig(
      FLAGS_scenario_traffic_light_unprotected_left_turn_config_file,
      &config_map_[ScenarioStatus::TRAFFIC_LIGHT_UNPROTECTED_LEFT_TURN]));
  ACHECK(Scenario::LoadConfig(
      FLAGS_scenario_traffic_light_unprotected_right_turn_config_file,
      &config_map_[ScenarioStatus::TRAFFIC_LIGHT_UNPROTECTED_RIGHT_TURN]));

  // valet parking
  ACHECK(Scenario::LoadConfig(FLAGS_scenario_valet_parking_config_file,
                              &config_map_[ScenarioStatus::VALET_PARKING]));

  // yield_sign
  ACHECK(Scenario::LoadConfig(FLAGS_scenario_yield_sign_config_file,
                              &config_map_[ScenarioStatus::YIELD_SIGN]));
}

// ScenarioStatus::ScenarioType ScenarioManager::SelectPullOverScenario(
//     const Frame& frame) {
//   const auto& scenario_config =
//       config_map_[ScenarioStatus::PULL_OVER].pull_over_config();
...
//         const auto hdmap_ptr = HDMapUtil::MapPtrForPlanning();
...
//     case ScenarioStatus::NNP_LANE_FOLLOW:
//       if (pull_over_scenario) {
//         return ScenarioStatus::PULL_OVER;
//       }
//       break;
//     case ScenarioStatus::BARE_INTERSECTION_UNPROTECTED:
//     case ScenarioStatus::EMERGENCY_PULL_OVER:
//     // case ScenarioStatus::PARK_AND_GO:
//     case ScenarioStatus::PULL_OVER:
//     case ScenarioStatus::STOP_SIGN_PROTECTED:
//     case ScenarioStatus::STOP_SIGN_UNPROTECTED:
//     case ScenarioStatus::TRAFFIC_LIGHT_PROTECTED:
//     case ScenarioStatus::TRAFFIC_LIGHT_UNPROTECTED_LEFT_TURN:
//     case ScenarioStatus::TRAFFIC_LIGHT_UNPROTECTED_RIGHT_TURN:
...
ScenarioStatus::ScenarioType ScenarioManager::SelectPadMsgScenario() {
  const auto& pad_msg_driving_action =
      TL::planning::Frame::GetPadMsgDrivingAction();

  switch (pad_msg_driving_action) {
    case DrivingAction::PULL_OVER:
      if (FLAGS_enable_scenario_emergency_pull_over) {
        return ScenarioStatus::EMERGENCY_PULL_OVER;
      }
      break;
    case DrivingAction::STOP:
      if (FLAGS_enable_scenario_emergency_stop) {
        return ScenarioStatus::EMERGENCY_STOP;
      }
      break;
      // case DrivingAction::RESUME_CRUISE:
      //   if (current_scenario_->scenario_type() ==
      //           ScenarioStatus::EMERGENCY_PULL_OVER ||
      //       current_scenario_->scenario_type() ==
      //           ScenarioStatus::EMERGENCY_STOP) {
      //     return ScenarioStatus::PARK_AND_GO;
      //   }
      break;
    default:
      break;
...
bool ScenarioManager::CanSelectInterceptionScenario(const Frame& frame) {
  const auto& reference_line_info = frame.reference_line_info().front();
  return !(reference_line_info.IsChangeLanePath() &&
           reference_line_info.IsDrivable());
}

ScenarioStatus::ScenarioType ScenarioManager::SelectInterceptionScenario(
    const Frame& frame) {
  ScenarioStatus::ScenarioType scenario_type = default_scenario_type_;

  const hdmap::PathOverlap* traffic_sign_overlap = nullptr;
  const hdmap::PathOverlap* pnc_junction_overlap = nullptr;
  ReferenceLineInfo::OverlapType overlap_type = ReferenceLineInfo::CLEAR_AREA;
  ...
  for (const auto& overlap : first_encountered_overlaps) {
    if (overlap.first == ReferenceLineInfo::SIGNAL ||
      ...
      traffic_sign_overlap = &overlap.second;
      break;
    }
    if (overlap.first == ReferenceLineInfo::PNC_JUNCTION) {
      pnc_junction_overlap = &overlap.second;
    }
  }

  // the traffic_sign is ads's favorite
  const double adc_end_s = reference_line_info.AdcSlBoundary().end_s();
  if ((traffic_sign_overlap != nullptr) && (pnc_junction_overlap != nullptr)) {
    ...
    if (s_diff >= kJunctionDelta) {
      if (pnc_junction_overlap->end_s > traffic_sign_overlap->end_s ||
          (pnc_junction_overlap->start_s > traffic_sign_overlap->start_s &&
           pnc_junction_overlap->end_s < traffic_sign_overlap->end_s) ||
          (adc_end_s > traffic_sign_overlap->start_s)) {
  ...
  } else if (pnc_junction_overlap != nullptr) {
    // bare intersection
...
ScenarioStatus::ScenarioType ScenarioManager::SelectStopSignScenario(
  ...
  switch (current_scenario_->scenario_type()) {
    ...
    case ScenarioStatus::PULL_OVER:
      if (stop_sign_scenario) {
        return stop_sign_all_way ? ScenarioStatus::STOP_SIGN_PROTECTED
                                 : ScenarioStatus::STOP_SIGN_UNPROTECTED;
      }
      break;
    case ScenarioStatus::BARE_INTERSECTION_UNPROTECTED:
    case ScenarioStatus::EMERGENCY_PULL_OVER:
...
ScenarioStatus::ScenarioType ScenarioManager::SelectTrafficLightScenario(
  ...
  for (const auto& traffic_light_overlap : next_traffic_lights) {
    ...
    auto signal_color =
        frame
            .GetSignalByTurnType(reference_line_info.GetPathTurnType(
    ...
    if (signal_color != perception::GREEN) {
      red_light = true;
      break;
    }
  }
  ...
  switch (current_scenario_->scenario_type()) {
    ...
    case ScenarioStatus::PULL_OVER:
      if (traffic_light_unprotected_left_turn_scenario) {
        return ScenarioStatus::TRAFFIC_LIGHT_UNPROTECTED_LEFT_TURN;
      } else if (traffic_light_unprotected_right_turn_scenario) {
        return ScenarioStatus::TRAFFIC_LIGHT_UNPROTECTED_RIGHT_TURN;
      } else if (traffic_light_protected_scenario) {
        return ScenarioStatus::TRAFFIC_LIGHT_PROTECTED;
      }
      break;
    case ScenarioStatus::BARE_INTERSECTION_UNPROTECTED:
    case ScenarioStatus::EMERGENCY_PULL_OVER:
...
ScenarioStatus::ScenarioType ScenarioManager::SelectYieldSignScenario(
    const Frame& frame, const hdmap::PathOverlap& yield_sign_overlap) {
  const auto& scenario_config =
      config_map_[ScenarioStatus::YIELD_SIGN].yield_sign_config();
  ...
  switch (current_scenario_->scenario_type()) {
    ...
    case ScenarioStatus::PULL_OVER:
      if (yield_sign_scenario) {
        return ScenarioStatus::YIELD_SIGN;
      }
      break;
    case ScenarioStatus::BARE_INTERSECTION_UNPROTECTED:
    case ScenarioStatus::EMERGENCY_PULL_OVER:
...
ScenarioStatus::ScenarioType ScenarioManager::SelectBareIntersectionScenario(
  ...
  switch (current_scenario_->scenario_type()) {
    ...
    case ScenarioStatus::PULL_OVER:
      if (bare_junction_scenario) {
        return ScenarioStatus::BARE_INTERSECTION_UNPROTECTED;
      }
      break;
    case ScenarioStatus::BARE_INTERSECTION_UNPROTECTED:
    case ScenarioStatus::EMERGENCY_PULL_OVER:
...
ScenarioStatus::ScenarioType ScenarioManager::SelectValetParkingScenario(
    const Frame& frame) {
  if (frame.local_view().GetFunctionManagerIn()->ta_pilot_mode() ==
      functionmanager::AVP) {
    return ScenarioStatus::VALET_PARKING;
  }

  return default_scenario_type_;
}

ScenarioStatus::ScenarioType ScenarioManager::SelectNCPLaneFollowScenario(
    const Frame& frame) {
  if (nullptr != frame.local_view().GetFunctionManagerOut() &&
    ...
    return ScenarioStatus::NCP_LANE_FOLLOW;
  }

  return default_scenario_type_;
}

// ScenarioStatus::ScenarioType ScenarioManager::SelectParkAndGoScenario(
//     const Frame& frame) {
//   bool park_and_go = false;
//   const auto& scenario_config =
//       config_map_[ScenarioStatus::PARK_AND_GO].park_and_go_config();
//   const auto vehicle_state_provider = injector_->vehicle_state();
//   common::VehicleState vehicle_state =
//   vehicle_state_provider->vehicle_state(); auto adc_point =
//   common::util::PointFactory::ToPointENU(vehicle_state);
...
//   const auto& routing = frame.local_view().routing;
//   const auto& routing_end =
//   *(routing->routing_request().waypoint().rbegin()); common::SLPoint
//   dest_sl; const auto& reference_line_info =
//   frame.reference_line_info().front(); const auto& reference_line =
...
//   const double adc_distance_to_dest = dest_sl.s() - adc_front_edge_s;
//   ADEBUG << "adc_distance_to_dest:" << adc_distance_to_dest;
//   // if vehicle is static, far enough to destination and (off-lane or not
//   on
//   // city_driving lane)
//   if (std::fabs(adc_speed) < max_abs_speed_when_stopped &&
//       adc_distance_to_dest > scenario_config.min_dist_to_dest() &&
//       (HDMapUtil::MapForPlanning().GetNearestLaneWithHeading(
...
void ScenarioManager::ScenarioDispatch(const Frame& frame) {
  ACHECK(!frame.reference_line_info().empty());
  ScenarioStatus::ScenarioType scenario_type = ScenarioStatus::NNP_LANE_FOLLOW;

  scenario_type = ScenarioDispatchNonLearning(frame);

  ADEBUG << "select scenario: "
         << ScenarioStatus::ScenarioType_Name(scenario_type);

  // update PlanningContext
  UpdatePlanningContext(frame, scenario_type);

  if (current_scenario_->scenario_type() != scenario_type ||
      (frame.GetIsStateChange() && frame.local_view().HasFunctionManagerIn() &&
...
// ScenarioStatus::ScenarioType ScenarioManager::ScenarioDispatchLearning() {
//   ////////////////////////////////////////
//   // learning model scenario
//   ScenarioStatus::ScenarioType scenario_type =
//       ScenarioStatus::LEARNING_MODEL_SAMPLE;
//   return scenario_type;
// }

ScenarioStatus::ScenarioType ScenarioManager::ScenarioDispatchNonLearning(
    const Frame& frame) {
  ////////////////////////////////////////
  // default: NNP_LANE_FOLLOW
  ScenarioStatus::ScenarioType scenario_type =
      SelectValetParkingScenario(frame);
  if (scenario_type == ScenarioStatus::VALET_PARKING) {
    return scenario_type;
  }

  ////////////////////////////////////////
  // Pad Msg scenario
  scenario_type = SelectPadMsgScenario();
  ...
  if (scenario_type == default_scenario_type_) {
    ...
    switch (current_scenario_->scenario_type()) {
      ...
      case ScenarioStatus::YIELD_SIGN:
        ...
        if (current_scenario_->GetStatus() !=
            Scenario::ScenarioState::STATUS_DONE) {
          scenario_type = current_scenario_->scenario_type();  // NOLINT
        }
        break;
      default:
        break;
  ...
  // if (scenario_type == default_scenario_type_ &&
  //     CanSelectInterceptionScenario(frame)) {
  ...
  //   }
  // }
  scenario_type = SelectNCPLaneFollowScenario(frame);
...
bool ScenarioManager::IsBareIntersectionScenario(
    const ScenarioStatus::ScenarioType& scenario_type) {
  return (scenario_type == ScenarioStatus::BARE_INTERSECTION_UNPROTECTED);
}

bool ScenarioManager::IsStopSignScenario(
    const ScenarioStatus::ScenarioType& scenario_type) {
  return (scenario_type == ScenarioStatus::STOP_SIGN_PROTECTED ||
          scenario_type == ScenarioStatus::STOP_SIGN_UNPROTECTED);
}

bool ScenarioManager::IsTrafficLightScenario(
    const ScenarioStatus::ScenarioType& scenario_type) {
  return (
      scenario_type == ScenarioStatus::TRAFFIC_LIGHT_PROTECTED ||
      scenario_type == ScenarioStatus::TRAFFIC_LIGHT_UNPROTECTED_LEFT_TURN ||
      scenario_type == ScenarioStatus::TRAFFIC_LIGHT_UNPROTECTED_RIGHT_TURN);
}

bool ScenarioManager::IsYieldSignScenario(
    const ScenarioStatus::ScenarioType& scenario_type) {
  return (scenario_type == ScenarioStatus::YIELD_SIGN);
}
...
void ScenarioManager::UpdatePlanningContext(
  ...
  // ValetParking scenario
  UpdatePlanningContextValetParkingScenario(frame, scenario_type);
}

// update: valet_parking status in PlanningContext
void ScenarioManager::UpdatePlanningContextValetParkingScenario(
    const Frame& frame, const ScenarioStatus::ScenarioType& scenario_type) {
  UNUSED(frame);
...
void ScenarioManager::UpdatePlanningContextEmergencyStopcenario(
  ...
  if (scenario_type != ScenarioStatus::EMERGENCY_STOP) {
...
void ScenarioManager::UpdatePlanningContextPullOverScenario(
  ...
  auto* pull_over = injector_->planning_context()
                        ->mutable_planning_status()
                        ->mutable_pull_over();
  if (scenario_type == ScenarioStatus::PULL_OVER) {
    pull_over->set_pull_over_type(PullOverStatus::PULL_OVER);
    pull_over->set_plan_pull_over_path(true);
    return;
  }
  if (scenario_type == ScenarioStatus::EMERGENCY_PULL_OVER) {
