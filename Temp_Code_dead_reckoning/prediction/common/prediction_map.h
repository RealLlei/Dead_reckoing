...
class PredictionMap {
 public:
  ...
  static std::string MapModuleName();
   ...
   */
  static double CurvatureOnLane(const std::string& lane_id, double s);
  ...
  /**
   * @brief Get a shared pointer to a section by lane ID.
   * @param id The ID of the target lane ID in the form of string.
   * @return A shared pointer to the lane with the input lane ID.
   */
  static std::shared_ptr<const hdmap::RoadSection> GetLaneSection(
      const std::string& lane_id);

  static bool IsSectionLaneFromLeft2Right(const hdmap::RoadSection& section);

  /**
   * @brief combined road id with section id to a string id.
   * @param lane_info target lane.
   * @return A string id, such as "123,0", "123" is road id, "0" is section id.
   */
  static std::string GetRoadSectionCombinedId(
      const std::shared_ptr<const hdmap::LaneInfo>& lane_info);

  /**
   * @brief combined road id with section id to a string id.
   * @param combined_id a string id, such as "123,0", "123" is road id, "0" is section id.
   */
  static bool GetRoadAndSectionIdFromCombinedId(const std::string& combined_id,
                                                std::string* road_id,
  ...
  static bool GetProjection(const Eigen::Vector2d& position,
                            const std::vector<std::string>& lane_ids, double* s,
                            double* l, int* idx);

  static bool GetLaneSequenceS(const std::vector<std::string>& lane_ids,
   ...
   * @param s The longitudinal coordinate of the position.
   * @param l The lateral coordinate of the position.
   * @param index The lane point index of the position.
   */
  static bool GetProjectionWithIndex(
      const Eigen::Vector2d& position,
      const std::shared_ptr<const hdmap::LaneInfo>& lane_info, double* s,
      double* l, int* index);
   ...
   */
  static bool HasNearbyLane(double x, double y, double radius);
   ...
   */
  static bool OnVirtualLane(const Eigen::Vector2d& position, double radius);

  static int GetNearestLane(
      double x, double y, std::shared_ptr<const hdmap::LaneInfo>* nearest_lane,
  ...
  static void OnLane(
      ...
      const std::tuple<double, double, bool, int, double, double, double>&
          params,
   ...
   */
  static std::shared_ptr<const TL::hdmap::LaneInfo> GetMostLikelyCurrentLane(
      const common::PointENU& position, double radius, double heading,
   ...
   */
  static bool NearJunction(const Eigen::Vector2d& point, double radius);
   ...
   */
  static bool InJunction(double x, double y, double radius);

  static bool InJunction(const Eigen::Vector2d& point, double radius);
   ...
   */
  static std::vector<std::shared_ptr<const TL::hdmap::JunctionInfo>>
  GetJunctions(const Eigen::Vector2d& point, double radius);
   ...
   * @return A list of junctions
   */
  static std::vector<std::shared_ptr<const TL::hdmap::PNCJunctionInfo>>
  GetPNCJunctions(const Eigen::Vector2d& point, double radius);
   ...
   */
  static bool SmoothPointFromLane(const std::string& id, double s, double l,
  ...
  static void NearbyLanesByCurrentLanes(
      ...
      int max_num_lane, double extra_s,
  ...
  static std::shared_ptr<const hdmap::LaneInfo> GetLeftNeighborLane(
      ...
      const Eigen::Vector2d& ego_position, double threshold);
  ...
  static bool IsLeftNeighborLane(
      ...
      const std::shared_ptr<const hdmap::LaneInfo>& curr_lane);
  ...
  /**
   * @brief Check if a lane is a left neighbor of the lane_sequence and get the index.
   * @param target_lane The lane to check if it is a left neighbor.
   * @param lane_sequence The current lane sequence.
   * @return The index of left neighbor of the lane_sequence.
   */
  static int GetLeftNeighborLaneIdx(
      const std::shared_ptr<const hdmap::LaneInfo>& target_lane,
  ...
  /**
   * @brief Check if a lane is a right neighbor of the lane_sequence and get the index.
   * @param target_lane The lane to check if it is a right neighbor.
   * @param lane_sequence The current lane sequence.
   * @return The index of right neighbor of the lane_sequence.
   */
  static int GetRightNeighborLaneIdx(
      const std::shared_ptr<const hdmap::LaneInfo>& target_lane,
      const LaneSequence& lane_sequence);
  ...
  static bool IsIdenticalLane(
      const std::shared_ptr<const hdmap::LaneInfo>& other_lane,
      const std::shared_ptr<const hdmap::LaneInfo>& curr_lane);
  ...
  static std::vector<std::shared_ptr<const hdmap::LaneInfo>> GetNearbyLanes(
      const common::PointENU& position, double nearby_radius);

  static std::shared_ptr<const hdmap::LaneInfo> LaneWithSide(
  ...
  /**
   * @brief Get the pointer to the lane with the smallest angle diff
   * @param The predecessor lane
   * @param The vector of lane infos
   * @return The pointer to the lane with the smallest average curvature
   */
  static std::shared_ptr<const hdmap::LaneInfo> LaneWithSmallestAngleDiff(
      const std::shared_ptr<const hdmap::LaneInfo>& curr_lane_info,
      const std::vector<std::shared_ptr<const hdmap::LaneInfo>>& lane_infos,
      size_t sample_size);
  ...
  /**
   * @brief Get the index of lane in the lane_sequence with the ID lane_id
   * @param The ID of the lane
   * @param The lane_sequence
   * @return The the index of lane in the lane_sequence
   */
  static int GetLaneInSequenceIndex(const std::string& lane_id,
                                    const LaneSequence& lane_sequence);

  /**
   * @brief Check if two lane sequence have same lane
   * @param The lane_sequence
   * @param The lane_sequence2
   * @return If two seq have same lane
   */
  static bool IsTwoSequenceHaveSameLane(const LaneSequence& lane_sequence,
                                        const LaneSequence& lane_sequence2);

  static bool IsDrivableLane(const hdmap::Lane::LaneType& lane_type);

  static bool IsDrivableLaneSequence(const LaneSequence& lane_seq);

  static bool IsTwoSequenceSame(const LaneSequence& lane_sequence,
                                const LaneSequence& lane_sequence2);

  static bool IsTwoSequenceConnected(const LaneSequence& target_seq,
                                     const LaneSequence& curr_seq);

  static bool IsSequenceContainsAnother(const LaneSequence& target_seq,
                                        const LaneSequence& curr_seq);

  static bool IsSequenceExtendsAnother(const LaneSequence& target_seq,
                                       const LaneSequence& curr_seq);

  static bool GetProjLanePointHeading(const LaneSequence& lane_sequence,
                                      double* heading);

  static bool GetProjLanePointWidth(const LaneSequence& lane_sequence,
                                    double* width);

  static bool IsPolyInLaneSequenceLatRange(const LaneSequence& lane_sequence,
                                           double lat_dist);

  static bool IsEmergencyLaneSequence(const LaneSequence& lane_sequence);

  static bool IsInMergeLaneSequence(const LaneGraph& lane_graph);

  static bool IsLaneLineSolid(
      bool left_side, double s,
      const std::shared_ptr<const hdmap::LaneInfo>& lane_info);

  static bool GetLaneIdFromLaneSequenceByS(const LaneSequence& lane_sequence,
                                           double s, std::string* lane_id);

  static bool LaneSequenceContainsEgoLaneSequence(
 ...
 private:
  static std::shared_ptr<const hdmap::LaneInfo> GetNeighborLane(
      ...
      const std::vector<std::string>& neighbor_lane_ids, double threshold);
};
