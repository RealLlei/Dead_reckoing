...
DEFINE_double(prediction_trajectory_time_length_freemove, 3.0,
              "Time length of predicted trajectory (in seconds)");
DEFINE_double(prediction_trajectory_time_length_pre_freemove, 3.0,
              "Time length of predicted trajectory (in seconds)");
DEFINE_double(prediction_pedestrian_gaussian_info_sd_upperbound, 5.0,
              "standard deviation upperbound of predicted pedestrian gaussian "
              "info (in meters)");
DEFINE_double(prediction_pedestrian_gaussian_info_sd_lowerbound, 0.1,
              "standard deviation lowerbound of predicted pedestrian gaussian "
              "info (in meters)");
DEFINE_double(prediction_pedestrian_gaussian_info_sd_scale, 1.0,
              "standard deviation scale of predicted pedestrian gaussian info");
DEFINE_double(min_prediction_trajectory_spatial_length, 100.0,
              "Minimal spatial length of predicted trajectory");
DEFINE_double(min_ego_trajectory_spatial_length, 200.0,
              "Minimal spatial length of ego trajectory");
...
DEFINE_double(min_poly_time_length, 0.2,
              "Minimal polymial length of predicted trajectory");

DEFINE_double(vehicle_max_linear_acc, 5.0,
              "Upper bound of vehicle linear acceleration");
DEFINE_double(vehicle_min_linear_acc, -5.0,
              "Lower bound of vehicle linear deceleration");
DEFINE_double(vehicle_max_linear_acc_longitude_poly, 5.0,
              "Upper bound of vehicle linear acceleration");
DEFINE_double(vehicle_min_linear_acc_longitude_poly, -5.0,
              "Lower bound of vehicle linear deceleration");
DEFINE_double(vehicle_max_linear_acc_freemove, 1.0,
              "Upper bound of vehicle linear acceleration");
DEFINE_double(vehicle_min_linear_acc_freemove, -1.0,
              "Lower bound of vehicle linear deceleration");
...
// Map
DEFINE_double(lane_search_radius, 5.0, "Search radius for a candidate lane");
DEFINE_double(lane_search_radius_in_junction, 5.0,
...
DEFINE_bool(build_lanegraph_consider_u_turn, false,
            "If consider u-turn when building lanegraph.");
...
DEFINE_bool(enable_all_pedestrian_caution_in_front, true,
...
// Obstacle Filter
DEFINE_bool(enable_percpetion_filter, true,
            "If enable the perception obstacle filter.");
DEFINE_bool(enable_perception_sensor_filter, true,
            "If enable the perception sensor filter.");
DEFINE_bool(enable_perception_sensor_radar_filter, true,
            "If enable the radar filter.");
DEFINE_double(only_front_radar_range, 80.0,
              "The detect range for only one front radar");
DEFINE_int32(only_front_radar_frame_num, 3,
             "The detect range for only one front radar");
DEFINE_bool(enable_perception_sensor_lidar_filter, true,
            "If enable the lidar filter.");
DEFINE_double(only_front_lidar_range, 20.0,
              "The detect range for only one front lidar");
DEFINE_int32(only_front_lidar_frame_num, 3,
             "The detect range for only one front lidar");
DEFINE_bool(enable_perception_outofmap_filter, false,
            "If enable the  out of map filter.");

// Kalman
DEFINE_double(converged_scale, 0.01,
              "The converged scale of the kalman filter");
DEFINE_double(unconverged_scale, 1.0,
              "The unconverged scale of the kalman filter");

// Obstacle features
DEFINE_int32(TypeWindowSize, 5, "The window size for obstacle type");
DEFINE_double(TruckAreaThreshold, 20.0, "The area threshold for truck");
DEFINE_double(TruckMFThreshold, 0.79, "The MF threshold for truck");

DEFINE_int32(ego_vehicle_id, -1, "The obstacle ID of the ego vehicle.");
DEFINE_double(scan_length, 400.0, "The length of the obstacles scan area");
DEFINE_double(scan_width, 20.0, "The width of the obstacles scan area");
DEFINE_double(scan_radius, 70.0, "The width of the obstacles round scan area");
...
DEFINE_int32(max_still_obstacle_history_length, 10,
             "Min # historical frames for still obstacles");
DEFINE_double(still_obstacle_speed_threshold, 0.2,
              "Speed threshold for still obstacles");
DEFINE_double(still_bicycle_speed_threshold, 0.2,
              "Speed threshold for still pedestrians");
DEFINE_double(still_pedestrian_speed_threshold, 0.2,
              "Speed threshold for still pedestrians");
DEFINE_double(still_obstacle_speed_threshold_upper, 0.4,
              "Speed threshold for still obstacles");
DEFINE_double(still_unknown_speed_threshold, 0.2,
...
DEFINE_double(extra_long_veh_length_threshold, 12.0,
              "Length threshold for extra long vehicle");
DEFINE_double(slow_obstacle_speed_threshold, 2.0,
              "Speed threshold for slow obstacles");
DEFINE_double(suppose_to_stop_speed_threshold, 5.5,
              "Speed threshold for stop obstacles");
DEFINE_double(suppose_to_stop_acc_threshold, -0.2,
              "acc threshold for stop obstacles");
DEFINE_double(max_history_time, 3.0, "Obstacles' maximal historical time.");
...
#ifdef ISMDC
DEFINE_string(trajnet_evaluator_file, "data/model/walkerTrajNet.om",
              "trajectory prediction model file for pedestrian evaluator");
DEFINE_string(evaluator_vehicle_tnt_file, "data/model/tnt.om",
              "tnt model file for vehicle evaluator");
#elif defined(ISORIN)
DEFINE_string(trajnet_evaluator_file, "data/model/walkerTrajNet.engine",
              "trajectory prediction model file for pedestrian evaluator");
DEFINE_string(evaluator_vehicle_tnt_file, "data/model/tnt.engine",
              "tnt model file for vehicle evaluator");
DEFINE_string(evaluator_qcnet_fullmodel_file, "data/model/qcnet.engine",
              "qcnet model file for qcnet evaluator");
#else
DEFINE_string(trajnet_evaluator_file, "data/model/walkerTrajNet.onnx",
              "trajectory prediction model file for pedestrian evaluator");
DEFINE_string(evaluator_vehicle_tnt_file, "data/model/tnt.onnx",
              "tnt model file for vehicle evaluator");
DEFINE_string(evaluator_qcnet_fullmodel_file, "data/model/qcnet.onnx",
              "qcnet model file for qcnet evaluator");
#endif
...
DEFINE_string(torch_vehicle_vectornet_file, "data/vectornet.bin",
              "vectornet_gpu_model");
DEFINE_string(torch_vehicle_vectornet_cpu_file, "data/vectornet_cpu.bin",
              "vectornet_cpu_model");
DEFINE_string(torch_vehicle_jointly_model_file, "data/joinlty_model.bin",
              "joinlty_model_model");
DEFINE_string(torch_vehicle_jointly_model_cpu_file,
              "data/joinlty_model_cpu.bin", "joinlty_model_model");
...
DEFINE_bool(adjust_vehicle_heading_by_lane, false,
...
DEFINE_uint32(freemove_frame_onlane, 10, "freemove_frame when obstalce onlane");

DEFINE_string(mdc_vehicle_cruise_go_file, "data/cruise_go_vehicle_model.om",
              "Vehicle cruise go model file");
...
DEFINE_double(intrusion_width_ratio, 0.1,
              "The area of the obstacle intruding into the target lane "
              "determines the timing of the release trajectory");
DEFINE_double(cost_min_preview_time, 0.3,
              "The minimum preview time for cost calculation");
DEFINE_double(cost_max_preview_time, 0.8,
              "The max preview time for cost calculation");
DEFINE_double(cost_min_adjustment, -0.1,
              "The min adjustment for cost calculation");
DEFINE_double(cost_max_adjustment, 0.1,
              "The max adjustment for cost calculation");
DEFINE_double(dist_obs2merge_lane, 50.0, "The dist obs to merge lane");
DEFINE_bool(enable_output_parallel_traj, true,
            "Output parallel trajectory for small prob lane sequence");
...
DEFINE_double(lane_change_dist, 20.0, "Lane change distance with ADC");
...
DEFINE_double(default_time_to_lat_end_state, 3.0,
...
DEFINE_double(first_order_K, 0.5, "The first order K");
...
DEFINE_double(road_distance, 150.0,
...
DEFINE_bool(enable_build_vectornet_graph, false,
            "enable build vector graph for obstacle");
DEFINE_bool(save_routing_lane_ids, true, "whether to save routing lane ids");
DEFINE_int32(map_encoding_interval, 5, "map feature update interval");

DEFINE_int32(point_num, 1000, "point num in qcnet matrix");
DEFINE_int32(tnt_batch_size, 1, "batch size for TNT model");
DEFINE_int32(polyline_num, 100, "polyline num in qcnet/vectornet matrix");
DEFINE_int32(polyline_length, 20, "polyline length in vectornet matrix");
DEFINE_int32(vector_length, 10, "vector length in vectornet matrix");
DEFINE_int32(goals_2D_num, 10, "goals 2D num for TNT model");
DEFINE_int32(pred_num, 6, "Number of predicted trajectories in TNT model");
DEFINE_uint32(
    pred_vector_length, 6,
    "element in TNT output ,include index,score,pred_x,pred_y,goal_x,goal_y");

// dbn model
DEFINE_string(dbn_model_file, "data/model/dbn_model.bin",
              "dbn model file for predictor");
DEFINE_string(long_vehicle_dbn_model_file,
              "data/model/long_vehicle_dbn_model.bin",
              "dbn model file for predictor");
DEFINE_string(dbn_model_file_minieye, "data/model/dbn_model_minieye.bin",
              "dbn model file for predictor");
DEFINE_string(long_vehicle_dbn_model_file_minieye,
              "data/model/long_vehicle_dbn_model_minieye.bin",
              "dbn model file for predictor");
DEFINE_string(dbn_junction_model_file, "data/model/dbn_junction_model.bin",
              "dbn junction model file for predictor");
DEFINE_string(dbn_junction_exit_model_file,
              "data/model/dbn_junction_exit_model.bin",
              "dbn junction model file for predictor");
DEFINE_int32(obs_id_for_dbn_debug, -1, "obs id for dbn predictor debug");
