...
void FeatureOutput::InsertDataForLearning(
    ...
    const std::string& category, const LaneSequence* lane_sequence_ptr) {
  auto data_for_learning =
      list_data_for_learning_.mutable_data_for_learning()->Add();
...
void FeatureOutput::WriteFeatureProto() {
  ...
  if (features_.feature().empty()) {
    ...
    std::string timestr = UnixSecondsToString(time(nullptr), "%Y%m%d%H%M%S");
    const std::string file_name =
        absl::StrCat(FLAGS_prediction_data_dir, "/feature.", timestr, ".",
                     idx_feature_, ".bin");
    common::SetProtoToBinaryFile(features_, file_name);
    features_.Clear();
    ++idx_feature_;
  }
}
...
void FeatureOutput::WriteDataForLearning() {
  ...
  if (list_data_for_learning_.data_for_learning().empty()) {
    ...
    std::string timestr = UnixSecondsToString(time(nullptr), "%Y%m%d%H%M%S");
    const std::string file_name =
        absl::StrCat(FLAGS_prediction_data_dir, "/datalearn.", timestr, ".",
                     idx_learning_, ".bin");
    common::SetProtoToBinaryFile(list_data_for_learning_, file_name);
    list_data_for_learning_.Clear();
    ++idx_learning_;
  }
}
...
void FeatureOutput::WritePredictionResult() {
  ...
  if (list_prediction_result_.prediction_result().empty()) {
    ...
    common::SetProtoToBinaryFile(list_prediction_result_, file_name);
    list_prediction_result_.Clear();
...
void FeatureOutput::WriteFrameEnv() {
  ...
  if (list_frame_env_.frame_env().empty()) {
    ...
    common::SetProtoToBinaryFile(list_frame_env_, file_name);
    list_frame_env_.Clear();
    ++idx_frame_env_;
  }
}
...
void FeatureOutput::WriteDataForTuning() {
  ...
  common::SetProtoToBinaryFile(list_data_for_tuning_, file_name);
  list_data_for_tuning_.Clear();
  ++idx_tuning_;
}
