...
void GenerateDataForLearning() {
  TL::hdmap::HDMapUtil::ReloadMaps();
  auto hd_map = hdmap::HDMapUtil::LoadLocalMapPtr();
  if (!hd_map) {
  ...
  if (!MessageProcess::Init(container_manager.get(), &predictor_manager,
                            &inference_manager, prediction_conf)
           .ok()) {
    return;
  }
  ...
  for (const auto& input : inputs) {
    ...
    for (std::size_t i = 0; i < offline_bags.size(); ++i) {
      AINFO << "\tProcessing: [ " << i << " / " << offline_bags.size()
            << " ]: " << offline_bags[i];
      CyberRecordProcess::ProcessOfflineData(
          prediction_conf, container_manager, &predictor_manager,
...
::prediction
  ...
  TL::prediction::GenerateDataForLearning();
  return 0;
}
