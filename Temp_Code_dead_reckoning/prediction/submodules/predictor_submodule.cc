...
#include "planning/prediction/submodules/predictor_submodule.h"
...
#include "planning/proto/adapter_config.pb.h"
...
bool PredictorSubmodule::Proc(
  ...
  const TL::common::Header& perception_header =
      perception_obstacles->header();
  const TL::common::ErrorCode& perception_error_code =
      perception_obstacles->error_code();
  const TL::common::Time& frame_start_time =
  ...
  const double end_time = Clock::NowInMicroseconds();
  ADEBUG << "End to end time = " << end_time - frame_start_time << " ms";
