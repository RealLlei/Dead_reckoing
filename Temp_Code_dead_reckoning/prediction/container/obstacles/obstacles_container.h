...
#include "planning/prediction/container/container.h"
...
#include "planning/prediction/submodules/submodule_output.h"
...
class ObstaclesContainer : public Container {
 public:
   ...
   * @brief Destructor
   */
  ~ObstaclesContainer() override = default;
  ...
  void Insert(const perception::PerceptionObstacles& perception_obstacles,
  ...
  void InsertPerceptionObstacle(
      ...
      double timestamp, ScenarioManager* scenario_manager = nullptr);

  /**
   * @brief Insert a feature proto message into the container
   * @param feature proto message
   */
  void InsertFeatureProto(Feature* feature);

  /**
   * @brief Build lane graph for obstacles
   */
  void BuildLaneGraph();
  void BuildEgoLaneGraph();
  ...
  /**
   * @brief Build junction map
   */
  void BuildJunctionMap();
  ...
  const TL::perception::PerceptionObstacle& GetPerceptionObstacle(int id);
  ...
  //   std::shared_ptr<JunctionAnalyzer> GetJunctionAnalyzer(
  //       const std::string& junction_id);

 private:
  Obstacle* GetObstacleWithLRUUpdate(int obstacle_id);
  ...
  static void SelectPreProcessMethod(
      const perception::PerceptionObstacle& perception_obstacle,
 ...
 private:
  ...
  std::unordered_map<std::string, std::shared_ptr<JunctionAnalyzer>>
      junction_analyzer_map_;
};
