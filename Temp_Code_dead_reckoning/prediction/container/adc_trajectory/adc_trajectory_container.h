...
class ADCTrajectoryContainer : public Container {
 public:
   ...
   * @brief Destructor
   */
  ~ADCTrajectoryContainer() override = default;
 ...
 private:
  ...
  static std::string ToString(const std::unordered_set<std::string>& lane_ids);

  static std::string ToString(const std::vector<std::string>& lane_ids);
