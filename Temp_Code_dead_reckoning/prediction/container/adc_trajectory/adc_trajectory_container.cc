...
void ADCTrajectoryContainer::Insert(
  ...
  double time1 = common::Clock::NowInSeconds();
  ...
  double time2 = common::Clock::NowInSeconds();
  ADEBUG << "copy period = " << (time2 - time1) * 1000;

  // Find ADC lane sequence
  SetLaneSequence();
  ADEBUG << "Generate an ADC lane id sequence [" << ToString(adc_lane_seq_)
         << "].";
  double time3 = common::Clock::NowInSeconds();
  ADEBUG << "set lane period = " << (time3 - time2) * 1000;
  ...
  double time4 = common::Clock::NowInSeconds();
  ADEBUG << "set lane period = " << (time4 - time3) * 1000;
}
...
std::shared_ptr<const TL::hdmap::JunctionInfo>
...
bool ADCTrajectoryContainer::HasOverlap(
  ...
  return std::any_of(
      ...
      [this](const LaneSegment& lane_segment) {
