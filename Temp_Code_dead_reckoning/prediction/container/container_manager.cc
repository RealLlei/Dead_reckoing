...
// #include
// "planning/prediction/container/storytelling/storytelling_container.h"

namespace TL {
namespace prediction {

using TL::common::adapter::AdapterConfig;
using TL::common::adapter::AdapterManagerConfig;
...
std::unique_ptr<Container> ContainerManager::CreateContainer(
  ...
  if (type == AdapterConfig::PERCEPTION_OBSTACLES) {
    container_ptr = std::make_unique<ObstaclesContainer>();
  } else if (type == AdapterConfig::LOCALIZATION) {
    container_ptr = std::make_unique<PoseContainer>();
  } else if (type == AdapterConfig::PLANNING_TRAJECTORY) {
    container_ptr = std::make_unique<ADCTrajectoryContainer>();
  }
