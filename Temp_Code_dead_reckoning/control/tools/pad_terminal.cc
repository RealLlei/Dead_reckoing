...
using TL::control::DrivingAction;
using TL::control::PadMessage;
using apollo::cyber::CreateNode;
using apollo::cyber::Node;
using apollo::apollo::cyber::Reader;
using apollo::apollo::cyber::Writer;
...
class PadTerminal {
 public:
  ...
  void send(int cmd_type) {
    ...
    TL::common::util::FillHeader("terminal", &pad);
  ...
  void on_chassis(const Chassis& chassis) {
    ...
    if (chassis.driving_mode() == Chassis::EMERGENCY_MODE) {
      if (is_first_emergency_mode) {
        count_start = TL::common::Clock::NowInNanoseconds() / 1e3;
        is_first_emergency_mode = false;
        AINFO << "detect emergency mode.";
      } else {
        int64_t diff =
            TL::common::Clock::NowInNanoseconds() / 1e3 - count_start;
