...
TrajectoryAnalyzer::TrajectoryAnalyzer(
  ...
  header_time_ = planning_published_trajectory->header().data_stamp();
...
std::vector<TrajectoryPoint>& TrajectoryAnalyzer::trajectory_points() {
...
bool TrajectoryAnalyzer::ComputeReferenceInfo(
    const common::PathPoint& origin_point, double base_time, double ts,
    int horizon, double current_v, int state_size, Eigen::MatrixXd* reference,
    SimpleMPCDebug* debug) const {
  static double headtime_prev = 0;
  uint point_counter = 0;
  double point_s_prev = 0;
  ADEBUG << "path_points_size:" << trajectory_points_.size();
  for (auto point : trajectory_points_) {
    if (header_time_ != headtime_prev &&
        point.relative_time() > -0.00001 &
            point.relative_time() <= horizon * ts) {
      // ADEBUG << "point_counter:" << point_counter
      //        << "   p_t:" << point.relative_time()
      //        << "   p_x:" << point.path_point().x()
      //        << "   p_y:" << point.path_point().y()
      //        << "   p_s:" << point.path_point().s()
      //        << "   p_theta:" << point.path_point().theta();
    }
    point_counter++;
    if (point.path_point().s() > 50) {
      break;
    }
  }

  headtime_prev = header_time_;
  // double radius = 100;
  // double angle = 0;
  for (int i = 0; i <= horizon; i++) {
    auto p = QueryNearestPointByAbsoluteTime(base_time + i * ts);
    auto p_next = QueryNearestPointByAbsoluteTime(base_time + (i + 1) * ts);
    auto relative_point = PointTransformToRelativeCoordinate(origin_point, p);
    auto next_relative_point =
        PointTransformToRelativeCoordinate(origin_point, p_next);
    (*reference)(i * state_size + 0, 0) = relative_point.path_point().y();
    (*reference)(i * state_size + 1, 0) = (next_relative_point.path_point().y() -
                                        relative_point.path_point().y()) /
                                       ts;
    (*reference)(i * state_size + 2, 0) = relative_point.path_point().theta();
    (*reference)(i * state_size + 3, 0) =
        (next_relative_point.path_point().theta() -
         relative_point.path_point().theta()) /
        ts;
    (*reference)(i * state_size + 4, 0) = p.path_point().s();
    (*reference)(i * state_size + 5, 0) = p.v();

    // test circle simulation
    // reference(i * state_size + 0, 0) = radius * sin(angle);
    // reference(i * state_size + 1, 0) = 0;
    // reference(i * state_size + 2, 0) = angle;
    // reference(i * state_size + 3, 0) = 0;
    // angle +=  10 * ts / radius;
    // auto path_point = debug->add_reference_state();
    // path_point->set_x(relative_point.path_point().x());
    // path_point->set_y(relative_point.path_point().y());
    // path_point->set_theta(relative_point.path_point().theta());

    // reference(i * state_size + 0, 0) = relative_point.path_point().y();
    // reference(i * state_size + 1, 0) =
    //     0;  // current_v * sin(relative_point.path_point().theta());
    // reference(i * state_size + 2, 0) = relative_point.path_point().theta();
    // reference(i * state_size + 3, 0) =
    //     0;  // current_v * relative_point.path_point().kappa();
    // reference(i * state_size + 4, 0) = p.path_point().s();
    // reference(i * state_size + 5, 0) = p.v();
  }
  //   ADEBUG << "reference matrix" << reference;
  return true;
}

common::TrajectoryPoint TrajectoryAnalyzer::PointTransformToRelativeCoordinate(
    const common::PathPoint& origin_point,
    const common::TrajectoryPoint& p) const {
  std::vector<common::TrajectoryPoint> relative_trajectory(trajectory_points_);
  auto cos_theta_diff = std::cos(-origin_point.theta());
  auto sin_theta_diff = std::sin(-origin_point.theta());
  common::TrajectoryPoint relative_point;
  relative_point.CopyFrom(p);
  auto x = p.path_point().x();
  auto y = p.path_point().y();
  auto theta = p.path_point().theta();
  auto tx = x - origin_point.x();
  auto ty = y - origin_point.y();
  auto x_new = tx * cos_theta_diff - ty * sin_theta_diff;
  auto y_new = tx * sin_theta_diff + ty * cos_theta_diff;
  auto theta_new = common::math::NormalizeAngle(theta - origin_point.theta());
  relative_point.mutable_path_point()->set_x(x_new);
  relative_point.mutable_path_point()->set_y(y_new);
  relative_point.mutable_path_point()->set_theta(theta_new);
