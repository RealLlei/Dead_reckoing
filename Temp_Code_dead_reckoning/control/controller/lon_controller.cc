...
// #include "localization/common/localization_gflags.h"
...
Status LonController::ComputeControlCommand(control::ControlCommand* cmd) {
  localization_ = &(injector_->local_view()->localization());
  chassis_ = &(injector_->local_view()->chassis());
  trajectory_message_ = &(injector_->local_view()->trajectory());
  ...
  if (trajectory_analyzer_ == nullptr ||
      trajectory_analyzer_->seq_num() !=
          trajectory_message_->header().seq()) {
  ...
  if (trajectory_message_->gear() == soc::Chassis::GEAR_REVERSE) {
  ...
  if ((trajectory_message_->trajectory_type() ==
       TL::planning::ADCTrajectory::NORMAL) &&
    ...
    acceleration_cmd =
        (chassis_->gear_location() == soc::Chassis::GEAR_REVERSE)
  ...
  if (std::fabs(cmd->debug().simple_lat_debug().steering_position() -
                cmd->debug().simple_lat_debug().steer_angle()) > 50.0 &&
      std::fabs(cmd->debug().simple_lon_debug().current_speed()) < 1.0) {
    acceleration_cmd =
        (chassis_->gear_location() == soc::Chassis::GEAR_REVERSE)
            ? std::max(acceleration_cmd,
                       -lon_controller_conf.standstill_acceleration())
            : std::min(acceleration_cmd,
                       lon_controller_conf.standstill_acceleration());
    ADEBUG << "Set steering angle when stoping";
    debug->set_is_full_stop(true);
  }
  ...
  if (std::fabs(injector_->vehicle_state()->linear_velocity()) <=
      ...
      chassis_->gear_location() == soc::Chassis::GEAR_NEUTRAL) {
...
void LonController::ComputeLongitudinalErrors(
  ...
  double current_control_time = common::Clock::NowInSeconds();
...
void LonController::GetPathRemain(SimpleLongitudinalDebug* debug) {
  ...
  if (trajectory_message_->gear() == soc::Chassis::GEAR_DRIVE) {
