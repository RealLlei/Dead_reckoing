...
DEFINE_string(planning_fct_topic, "/apollo/planning/fct",
              "planning fct topic name");
...
DEFINE_string(lanemarkers_laneline_topic,
              "/apollo/planning/LanemarkersLaneLine",
              "lanemarkerdebug topic name");
DEFINE_string(map_state_data_topic, "/apollo/hdmap/MapStateData",
              "map_state_data topic name");
DEFINE_string(perception_camera_lane_topic, "/result/camera/lane",
              "perception camera lane topic name");
DEFINE_string(perception_fusion_lane_topic, "/result/fusion/lane",
              "perception fusion lane topic name");
DEFINE_string(perception_detection_obstacle_topic,
              "/result/camera/detection/obstacle",
              "perception detection obstacle topic name");
DEFINE_string(perception_track_obstacle_topic, "/result/camera/track/obstacle",
              "perception track obstacle topic name");
DEFINE_string(perception_fusion_obstacle_topic, "/result/fusion/obstacle",
              "perception fusion obstacle topic name");
DEFINE_string(perception_lidar_obstacle_topic, "/result/lidar/obstacle",
              "perception lidar obstacle topic name");
...
DEFINE_string(image_front_topic, "/sensor/camera/front_narrow/image",
              "front camera image topic name for obstacles from camera");
DEFINE_string(image_front_wide_topic, "/sensor/camera/front_wide/image",
              "front camera image topic name for obstacles from camera");
DEFINE_string(image_front_left_topic, "/sensor/camera/front_left/image",
              "front camera image topic name for obstacles from camera");
DEFINE_string(image_front_right_topic, "/sensor/camera/front_right/image",
              "front camera image topic name for obstacles from camera");
DEFINE_string(image_rear_left_topic, "/sensor/camera/rear_left/image",
              "front camera image topic name for obstacles from camera");
DEFINE_string(image_rear_right_topic, "/sensor/camera/rear_right/image",
              "front camera image topic name for obstacles from camera");
DEFINE_string(image_rear_topic, "/sensor/camera/rear/image",
              "rear camera image topic name for obstacles from camera");
DEFINE_string(
    image_fisheye_front_topic, "/sensor/camera/fisheye_front/image",
    "front fisheye camera image topic name for obstacles from camera");
DEFINE_string(image_fisheye_left_topic, "/sensor/camera/fisheye_left/image",
              "left fisheye camera image topic name for obstacles from camera");
DEFINE_string(
    image_fisheye_right_topic, "/sensor/camera/fisheye_right/image",
    "right fisheye camera image topic name for obstacles from camera");
DEFINE_string(image_fisheye_rear_topic, "/sensor/camera/fisheye_rear/image",
              "rear fisheye camera image topic name for obstacles from camera");
DEFINE_string(
    image_fisheye_stitcher_topic, "/sensor/camera/fisheye_stitcher/image",
    "stitcher fisheye camera image topic name for obstacles from camera");
DEFINE_string(pointcloud_lidar_topic, "/sensor/lidar/huawei",
              "pointcloud topic name");
...
DEFINE_string(navigation_hdmap_topic, "/apollo/navigation_hdmap",
              "relative map");
DEFINE_string(navigation_topic, "/apollo/navigation", "navigation");
DEFINE_string(debug_related_data_topic, "/apollo/debug_related_data",
              "debug_related_data");
...
DEFINE_string(perception_parkingin_topic, "/perception/parking_slots",
              "openspace scene perception parking in topic name");
DEFINE_string(ehp_topic, "/amap/ehp", "amap ehp");
DEFINE_string(without_lane_line_topic, "/apollo/without_lane",
              "localview topic");

DEFINE_string(localization_ins_topic, "/apollo/localization/ins",
              "localization ins module trajectory");
DEFINE_string(localization_wo_topic, "/apollo/localization/wo",
              "localization wo module trajectory");
DEFINE_string(localization_mm_topic, "/apollo/localization/mm",
              "localization mm module trajectory");
DEFINE_string(localization_vio_topic, "/apollo/localization/vio",
              "localization vio module trajectory");

DEFINE_string(compressed_image_front_narrow_topic,
              "/sensor/camera/front_narrow/compressed/image",
              "front compressed image topic name for obstacles from camera");
DEFINE_string(compressed_image_front_topic,
              "/sensor/camera/front_wide/compressed/image",
              "front compressed image topic name for obstacles from camera");
DEFINE_string(compressed_image_front_left_topic,
              "/sensor/camera/front_left/compressed/image",
              "front compressed image topic name for obstacles from camera");
DEFINE_string(compressed_image_front_right_topic,
              "/sensor/camera/front_right/compressed/image",
              "front compressed image topic name for obstacles from camera");
DEFINE_string(compressed_image_rear_topic,
              "/sensor/camera/back/compressed/image",
              "rear compressed image topic name for obstacles from camera");

DEFINE_string(nns_route_topic, "/apollo/avp/nns_route", "topic of nns route");
DEFINE_string(nns_location_topic, "/apollo/avp/nns_location",
              "topic of nns location");
DEFINE_string(compressed_image_rear_left_topic,
              "/sensor/camera/back_left/compressed/image",
              "rear compressed image topic name for obstacles from camera");
DEFINE_string(compressed_image_rear_right_topic,
              "/sensor/camera/back_right/compressed/image",
              "rear compressed image topic name for obstacles from camera");

DEFINE_string(
    compressed_image_fisheye_front_topic,
    "/sensor/camera/fisheye_front/compressed/image",
    "fisheye front compressed image topic name for obstacles from camera");
DEFINE_string(
    compressed_image_fisheye_left_topic,
    "/sensor/camera/fisheye_left/compressed/image",
    "fisheye left compressed image topic name for obstacles from camera");
DEFINE_string(
    compressed_image_fisheye_right_topic,
    "/sensor/camera/fisheye_right/compressed/image",
    "fisheye right compressed image topic name for obstacles from camera");
DEFINE_string(
    compressed_image_fisheye_rear_topic,
    "/sensor/camera/fisheye_rear/compressed/image",
    "fisheye rear compressed image topic name for obstacles from camera");
DEFINE_string(
    compressed_image_fisheye_stitcher_topic,
    "/sensor/camera/fisheye_stitcher/compressed/image",
    "fisheye stitcher compressed image topic name for obstacles from camera");

DEFINE_string(TL_routing_response_topic, "/TL/routing_response",
              "routing response topic name");
DEFINE_string(TL_localization_topic, "/TL/localization/pose",
              "localization topic name");
DEFINE_string(TL_perception_obstacle_topic, "/TL/perception/obstacles",
              "perception obstacle topic name");
DEFINE_string(TL_prediction_topic, "/TL/prediction",
              "prediction topic name");
DEFINE_string(TL_planning_trajectory_topic, "/TL/planning",
              "planning trajectory topic name");
DEFINE_string(TL_chassis_topic, "/TL/soc/chassis", "chassis topic name");
DEFINE_string(TL_parkinglots_topic, "/TL/space/parkinglots",
              "parkinglots topic name");
DEFINE_string(TL_freespace_topic, "/TL/space/freespace",
              "freespace topic name");
DEFINE_string(TL_planning_fct_topic, "/TL/planning/fct",
              "planning fct topic name");
DEFINE_string(TL_ehp_topic, "/TL/amap/ehp", "amap ehp");
DEFINE_string(TL_nns_route_topic, "/apollo/avp/nns_route",
              "topic of nns route");
DEFINE_string(TL_nns_location_topic, "/TL/avp/nns_location",
              "topic of nns location");
DEFINE_string(TL_lanemarkers_laneline_topic,
              "/TL/planning/LanemarkersLaneLine",
              "lanemarkerdebug topic name");
// NOLINTEND
