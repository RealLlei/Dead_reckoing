...
const Time Time::MAX = Time(std::numeric_limits<uint64_t>::max());  // NOLINT
const Time Time::MIN = Time(0);                                     // NOLINT

Time::Time(uint64_t nanoseconds) : nanoseconds_(nanoseconds) {}  // NOLINT

Time::Time(int nanoseconds)
    : nanoseconds_(static_cast<uint64_t>(nanoseconds)) {}  // NOLINT

Time::Time(double seconds)
    : nanoseconds_(static_cast<uint64_t>(seconds * 1000000000UL)) {}  // NOLINT

Time::Time(uint32_t seconds, uint32_t nanoseconds)
    : nanoseconds_(static_cast<uint64_t>(seconds) * 1000000000UL +
                   nanoseconds) {}  // NOLINT

Time::Time(const Time& other) : nanoseconds_(other.nanoseconds_) {}  // NOLINT

Time& Time::operator=(const Time& other) {  // NOLINT
...
uint64_t Time::ToMicrosecond() const {
  return static_cast<uint64_t>(nanoseconds_ / 1000);
}

uint64_t Time::ToMillisecond() const {
  return static_cast<uint64_t>(nanoseconds_ / 1000000);
}

std::string Time::ToString() const {
#ifdef ISMDC
  auto nano = std::chrono::microseconds(nanoseconds_);
#else
  auto nano = std::chrono::nanoseconds(nanoseconds_);
#endif
  auto tp = system_clock::time_point(nano);
...
#else
  // char date_time[128];
  std::array<char, 128> date_time{};
  strftime(date_time.data(), sizeof(date_time), "%F %T", ret);  // NOLINT
  ss << std::string(date_time.data()) << "." << std::setw(9)
...
#ifdef ISMDC
  auto nano = std::chrono::microseconds(time.ToNanosecond());
#else
