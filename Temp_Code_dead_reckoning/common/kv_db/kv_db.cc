...
DEFINE_string(kv_db_path, "/apollo/data/kv_db.sqlite",  // NOLINT
              "Path to Key-value DB file.");            // NOLINT
...
class SqliteWraper {
 public:
  static int Callback(void* data, int argc, char** argv,
                      char** col_name) {  // NOLINT
    boost::span<char*> argv_span(argv, argc);
    if (data != nullptr) {
      auto* data_str = static_cast<std::string*>(data);
      *data_str = argc > 0 ? argv_span[0] : "";
    }
    return 0;
  }
  ...
  SqliteWraper(const SqliteWraper&) = delete;
  SqliteWraper(SqliteWraper&&) = delete;
  SqliteWraper operator=(const SqliteWraper&) = delete;
  SqliteWraper operator=(const SqliteWraper&&) = delete;
