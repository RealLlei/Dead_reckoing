...
Example:
         ...
         $0 [debug/release/fast] [mdc/cyber/baidu] [all] [llvm]
    "
}
...
function build(){
    ...
    if [ ! -f "libeuropa_common.so" ]; then
        echo -e "\033[0;31m Build finished in $duration s, But can not find library libeuropa_common.so in build folder, please check! \033[0m"
        return 1
    fi
...
function main() {
    ...
    if [ $# -ge 2 ]; then
        ...
        case "$2" in
            ...
            orin)  PLATFORM="ORIN" && TARGET_PATH=${PROJECT_FOLDER}/release/orin/europa ;;
            x86) PLATFORM="x86" ;;
            20dv) PLATFORM="20dv" ;;
        esac
        case "$3" in
