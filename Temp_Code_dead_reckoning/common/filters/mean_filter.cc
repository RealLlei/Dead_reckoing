...
double MF::UpdateForAll(const double measurement) {
  ACHECK(initialized_);
  CHECK_LE(values_.size(), window_size_);
  CHECK_LE(min_candidates_.size(), window_size_);
  CHECK_LE(max_candidates_.size(), window_size_);
...
void MF::Insert(const double value) {
  ...
  sum_ += value;
  while (!min_candidates_.empty() && min_candidates_.back().second > value) {
    min_candidates_.pop_back();
  }
  min_candidates_.emplace_back(time_, value);
  while (!max_candidates_.empty() && max_candidates_.back().second < value) {
    max_candidates_.pop_back();
  }
  max_candidates_.emplace_back(time_, value);
}

double MF::ReturnNextValue() {
  if (values_.size() <= 1) {
    return 0.0;
  }
  return values_.at(1);
