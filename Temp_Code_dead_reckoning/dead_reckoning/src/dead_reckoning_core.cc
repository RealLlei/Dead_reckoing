...
#include "dead_reckoning/src/compat.h"
#include <iomanip>
#include "../common/clock_stub.h"
...
void DeadReckoningCore::PublishLocalization(const std::shared_ptr<LocalizationOutput>& localization) {
    // Fill simple LocalizationOutput struct instead of protobuf message
    localization->timestamp = chassis_received_->timestamp;
    Eigen::Vector3d euler = Quaternion2EulerZyx(local_state_.q_i_w);
    if (euler(2) < -M_PI) { euler(2) = euler(2) + 2 * M_PI; }
    else if (euler(2) > M_PI) { euler(2) = euler(2) - 2 * M_PI; }
    double heading_angle_pub = euler(2) + 0.5 * M_PI;
    if (heading_angle_pub > M_PI) { heading_angle_pub = heading_angle_pub - 2 * M_PI; }
    else if (heading_angle_pub < -M_PI) { heading_angle_pub = heading_angle_pub + 2 * M_PI; }

    localization->pos.x = -1.0 * local_state_.pos_w(1);
    localization->pos.y = local_state_.pos_w(0);
    localization->heading = heading_angle_pub;
    localization->speed = local_state_.vel_w.norm();

    ADEBUG << "euler_angles: " << FIXED << SETPRECISION(6) << euler(0) << ";, " << euler(1) << ";, " << euler(2);
    ADEBUG << "point x," << FIXED << SETPRECISION(3) << localization->pos.x << ",y," << localization->pos.y << ",heading," << localization->heading;

    // optional logging to file (preserve previous behavior)
    std::vector<double> temp(std::vector<double>(21, 0.0));
    temp[0] = chassis_received_->timestamp;
    temp[1] = localization->pos.x;
    temp[2] = localization->pos.y;
    temp[3] = local_state_.pos_w(2);
    temp[4] = local_state_.vel_w(0);
    temp[5] = local_state_.vel_w(1);
    temp[6] = local_state_.vel_w(2);
    temp[7] = local_state_.acc_w(0);
    temp[8] = local_state_.acc_w(1);
    temp[9] = local_state_.acc_w(2);
    temp[12] = localization->heading;
    temp[19] = wheel_speed_.rr_wheel_speed;
    temp[20] = wheel_counter_speed_.rr_wheel_speed;
#ifdef EM_DEBUG
    std::ofstream ofs("/home/liubei/dr_output_log.txt", std::ios::app);
    for (int ii = 0; ii < 21; ii++) { ofs << std::setprecision(16) << temp[ii] << '\t'; }
    ofs << std::endl;
    ofs.close();
#endif
}
